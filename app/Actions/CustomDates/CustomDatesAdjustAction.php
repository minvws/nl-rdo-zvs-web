...
readonly class CustomDatesAdjustAction
{
    ...
    public function execute(
        ...
        CustomDateLabel $currentCustomDateLabel,
        CustomDateLabel $newCustomDateLabel,
    ): void {
        $this->databaseManager->transaction(static function () use ($petitionType, $currentCustomDateLabel, $newCustomDateLabel): void {
            PetitionTypeCustomDateLabel::query()
                ->where('petition_type_id', $petitionType->id)
                ->where('date_label', $currentCustomDateLabel)
                ->update(['date_label' => $newCustomDateLabel]);
