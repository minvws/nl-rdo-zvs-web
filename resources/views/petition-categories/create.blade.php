@section('pageTitle', __('petition_category.create'))

<x-form-layout>
    <x-slot name="header">
        <h1>{{ __('petition_category.create') }}</h1>
    </x-slot>
    ...
    <section class="mt-5">
        <div class="visually-grouped">
            <form
                method="post"
                action="{{ route(RouteName::DEPARTMENTS_ADMIN_PETITION_CATEGORIES_CREATE, ['department' => $department]) }}">
                @csrf
                <div>
                    <x-input-label
                        for="name"
