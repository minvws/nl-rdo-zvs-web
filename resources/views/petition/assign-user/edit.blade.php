...
<div class="petition-property__block petition-property__block--active">
    @ifHtmx
        <header class="petition-property__header">
            ...
            <a
                href="{{ route(RouteName::DEPARTMENTS_PETITIONS_ASSIGN_USER_SHOW, ["department" => $petition->department, "petition" => $petition]) }}"
    ...
    <div class="petition-property__content">
        <form
            method="post"
            action="{{ route(RouteName::DEPARTMENTS_PETITIONS_ASSIGN_USER_UPDATE, ["department" => $petition->department, "petition" => $petition]) }}"
            hx-push-url="false"
            hx-swap="innerHTML"
            hx-target="#assign-user-block">
            @csrf
            ...
            <div class="form-input-group">
                ...
                <select
                    ...
                    @foreach ($users as $user)
                        <option
                            value="{{ $user->id }}"
                            @selected($user->id->toString() === Form::old("user_id", $petition->assigned_to?->toString()))>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
